...
REM エンコードオプションの事前定義
set "enc_desc[1]=エンコードなし    (原形式  無圧縮    最速        原サイズ    サイト依存)"
set "enc_desc[2]=GPU - H.264       (NVIDIA  無圧縮    標準速度    最大サイズ  高互換性)"
...
set "enc_desc[7]=エンコードなし    (原形式  無圧縮    最速        原サイズ    サイト依存)"
set "enc_desc[8]=AAC - MP3         (MP3     高音質    高速        小サイズ    高互換性)"
set "enc_desc[9]=PCM - WAV         (WAV     無圧縮    高速        大サイズ    高互換性)"
...
REM サイト名を取得
for /f "usebackq delims=" %%a in (`yt-dlp --cookies "%~dp0cookies.txt" --skip-download --get-filename -o "%%^(extractor^)s" "!url!" 2^>nul`) do (
    set "site_name=%%a"
)

REM タイトルを取得
for /f "usebackq delims=" %%a in (`yt-dlp --cookies "%~dp0cookies.txt" --skip-download --get-filename -o "%%^(title^)s" "!url!" 2^>nul`) do (
    set "video_title=%%a"
)
...
if "%encode_option%"=="7" (
    ...
    REM エンコードなしの場合
    set "download_cmd=!download_cmd! -f "bestvideo+bestaudio/best" --merge-output-format mkv --write-subs --all-subs --embed-subs --fixup detect_or_warn --add-metadata --embed-chapters"
) else (
    REM 動画エンコードオプション
    set "download_cmd=!download_cmd! -f "bestvideo+bestaudio/best" --merge-output-format mp4 --write-subs --all-subs --embed-subs --fixup detect_or_warn --add-metadata --prefer-ffmpeg"
...
if "!exit_code!"=="0" (
    ...
    REM エラーが発生した場合でも、ファイルが存在するか確認
    set "file_found=0"
    for /f "delims=" %%a in ('dir /b "!output_dir!\*.*" 2^>nul ^| findstr /v "\.temp\." ^| findstr /v "\.part$"') do (
        set "file_found=1"
    )
...
REM 一時ファイル削除
echo 一時ファイルを削除しています...
if exist "!output_dir!\*.vtt" del /q "!output_dir!\*.vtt" 2>nul
if exist "!output_dir!\*.jpg" del /q "!output_dir!\*.jpg" 2>nul
if exist "!output_dir!\*.webp" del /q "!output_dir!\*.webp" 2>nul
if exist "!output_dir!\*.json" del /q "!output_dir!\*.json" 2>nul
if exist "!output_dir!\*.png" del /q "!output_dir!\*.png" 2>nul
if exist "!output_dir!\*.temp.mp4" del /q "!output_dir!\*.temp.mp4" 2>nul
if exist "!output_dir!\*.part" del /q "!output_dir!\*.part" 2>nul
if exist "!output_dir!\*.ytdl" del /q "!output_dir!\*.ytdl" 2>nul
